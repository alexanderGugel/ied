...
// before the download of the initial "direct" dependency via "fetchDep" has
// been completed.
...
function installAtomic (dir, tmpDest, finalDest, pkg, cb) {
  async.parallel([
    download.bind(null, tmpDest, pkg.dist.tarball, pkg.dist.shasum),
...
function handleResolvedPkg (dir, pkg, cb) {
  ...
  fs.stat(finalDest, function (err) {
    ...
    async.series([
      ...
      fs.rename.bind(fs, tmpDest, finalDest)
    ], function (err) {
      cb(err, pkg)
    })
  })
