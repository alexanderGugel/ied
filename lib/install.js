...
function linkPkg (tmpDest, pkg, cb) {
  var dstPath = path.join(tmpDest, 'node_modules', pkg.name)
  var series = []
  var dirname = path.dirname(pkg.name)
  var isScoped = dirname !== '.'
  if (isScoped) {
    series.push(mkdirp.bind(null, path.join(tmpDest, 'node_modules', dirname)))
  }
  var srcPath = path.join(isScoped ? '../../..' : '../..', pkg.dist.shasum, 'package')
