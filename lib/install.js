...
function handleResolvedPkg (dir, pkg, cb) {
  ...
  fs.stat(finalDest, function (err) {
    ...
    async.series([
      ...
      // Include package.json's metadata.
      // `pkg` at this point just comes from the resolver, which only has
      // { name, version, uid, shasum, tarball }.
      pkg = assign({}, require(path.join(dir, pkg.uid, 'package.json')), pkg)
