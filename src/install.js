...
export function resolve (cwd, parentTarget) {
	return this::mergeMap(([name, version]) => {
		...
		return resolveFromNodeModules(parentTarget, _path)
			::util.catchByCode({ ENOENT: () => resolveFromRemote(parentTarget, _path, name, version, cwd) })
 ...
 */
export function resolveAll (cwd, targets = Object.create(null)) {
	return this::expand(({isEntry, target, pkgJson}) => {
